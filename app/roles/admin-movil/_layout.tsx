...
export default function AdminLayout() {
    ...
    // Estados Contraseña (Reutilizados para funcionalidad completa)
    ...
    const {
        notificaciones,
        marcarTodasLeidas
    } = useNotifications('admin'); // Rol Admin
    ...
    const loadUserData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Verificar reset después de cargar/actualizar los datos
    ...
    const loadDarkMode = async () => {
        ...
        } catch (error) { console.error('Error cargando modo oscuro:', error); }
    };
    ...
    const checkPasswordReset = async () => {
        ...
        } catch (error) { console.error('Error verificando pass:', error); }
    };
    ...
    const handlePhotoOptions = () => {
        ...
        if (userData?.foto_perfil) options.push({ text: 'Eliminar Foto', onPress: () => deletePhoto(), style: 'destructive' });
                ...
                { text: 'Cancelar', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            if (response.ok) {
                                await loadUserData();
                                eventEmitter.emit('profilePhotoUpdated'); // Notificar cambios
    ...
    const handleLogout = () => {
        Alert.alert('Cerrar Sesión', '¿Estás seguro que deseas salir?', [
            { text: 'Cancelar', style: 'cancel' },
            { text: 'Salir', style: 'destructive', onPress: async () => { await clearAll(); router.replace('/'); } },
        ]);
    };
    ...
    return (
        <>
            <Tabs
                ...
                screenOptions={{
                    headerShown: true,
                    headerStyle: { backgroundColor: theme.cardBg, borderBottomWidth: 1, borderBottomColor: theme.border },
                    ...
                    headerRight: () => (
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginRight: 8 }}>
                            <NotificationBell notificaciones={notificaciones} onMarcarTodasLeidas={marcarTodasLeidas} darkMode={darkMode} />
                            <TouchableOpacity onPress={() => setShowProfileDrawer(true)} style={styles.profileButton}>
                                {userData?.foto_perfil ? (
                                    <Image source={{ uri: userData.foto_perfil }} style={[styles.headerAvatar, { borderColor: theme.accent }]} />
                                ) : (
                                    <View style={[styles.headerAvatarPlaceholder, { backgroundColor: theme.accent, borderColor: theme.accent }]}>
                        ...
                        <ScrollView>
                            ...
                            <View style={[styles.profileSection, { borderBottomColor: theme.border }]}>
                                <TouchableOpacity style={styles.avatarContainer} onPress={() => handlePhotoOptions()}>
                                    {userData?.foto_perfil ? (
                                        <Image source={{ uri: userData.foto_perfil }} style={[styles.avatarImage, { borderColor: theme.accent }]} />
                                    ) : (
                                        <View style={[styles.avatar, { backgroundColor: theme.accent, borderColor: theme.accent }]}>
                                            <Text style={styles.avatarText}>{getInitials()}</Text>
                                        </View>
                                    )}
                                    <View style={[styles.cameraIcon, { backgroundColor: theme.accent, borderColor: '#fff' }]}>
                                        <Ionicons name="camera" size={16} color="#fff" />
                                    </View>
                                </TouchableOpacity>
                                <Text style={[styles.profileName, { color: theme.text }]}>{userData?.nombre} {userData?.apellido}</Text>
                                <Text style={[styles.profileEmail, { color: theme.textSecondary }]}>{userData?.email}</Text>
                            </View>

                            <View style={[styles.settingItem, { borderBottomColor: theme.border }]}>
                                <View style={styles.settingLeft}>
                                    <Ionicons name={darkMode ? 'moon' : 'sunny'} size={24} color={theme.accent} />
                                    <Text style={[styles.settingText, { color: theme.text }]}>Modo Oscuro</Text>
                                </View>
                                <Switch value={darkMode} onValueChange={toggleDarkMode} trackColor={{ false: '#767577', true: theme.accent }} thumbColor="#fff" />
                            </View>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    ...
                                    // router.push no existe en la interfaz que tengo, uso navigation o router.push raw string
                                    // Router se define arriba como router. 
                                    setTimeout(() => router.push('/roles/admin-movil/perfil' as any), 300);
                                }}
                            >
                                <View style={styles.settingLeft}>
                    ...
                    <View style={[styles.passwordModalContent, { backgroundColor: theme.cardBg }]}>
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.passwordForm}>
                                ...
                                {/* Requisitos */}
                                <View style={[styles.passwordRequirements, { backgroundColor: darkMode ? 'rgba(239, 68, 68, 0.1)' : 'rgba(239, 68, 68, 0.05)', borderColor: '#ef444430' }]}>
...
const styles = StyleSheet.create({
    ...
    headerAvatar: { width: 36, height: 36, borderRadius: 18, borderWidth: 2 },
    headerAvatarPlaceholder: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', borderWidth: 2 },
    ...
    drawerContent: { width: '100%', maxHeight: '80%', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingTop: 20 },
    drawerHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingBottom: 20, gap: 16 },
    ...
    avatarImage: { width: 80, height: 80, borderRadius: 40, borderWidth: 3 },
    avatar: { width: 80, height: 80, borderRadius: 40, justifyContent: 'center', alignItems: 'center', borderWidth: 3 },
    ...
    settingItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1 },
    settingLeft: { flexDirection: 'row', alignItems: 'center', gap: 12 },
    settingText: { fontSize: 16, fontWeight: '500' },
    passwordModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 },
    passwordModalContent: { borderRadius: 24, padding: 24, maxHeight: '80%' },
    passwordModalHeader: { alignItems: 'center', marginBottom: 24 },
    passwordIconContainer: { width: 64, height: 64, borderRadius: 32, justifyContent: 'center', alignItems: 'center', marginBottom: 16 },
    passwordModalTitle: { fontSize: 20, fontWeight: '700', marginBottom: 8, textAlign: 'center' },
    passwordModalSubtitle: { fontSize: 14, textAlign: 'center', lineHeight: 20 },
    passwordForm: { gap: 16 },
    passwordField: { gap: 8 },
    passwordLabel: { fontSize: 14, fontWeight: '600' },
    passwordInputContainer: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, gap: 12 },
    passwordInput: { flex: 1, fontSize: 16 },
    passwordButton: { paddingVertical: 16, borderRadius: 12, alignItems: 'center', marginTop: 8 },
    passwordButtonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
    passwordCancelButton: { paddingVertical: 16, borderRadius: 12, alignItems: 'center', borderWidth: 1, marginTop: 12 },
